...
<html lang="en">
    ...
    <body>
        @php
            $hasGallery = isset($Images) && $Images->count() > 0;
            $mainImg = !empty($ProjectsReferenceMain->image_main) ? asset($ProjectsReferenceMain->image_main) : asset('assets/backend/images/error/nopic.jpg');
        @endphp

        @if(Config::get('app.locale') == 'en')
        <!-- Slider -->
        <section class="project-section">
            <a href="/#projectref" class="btnCloseProject"></a>
            <div id="carousel" class="carousel slide" data-ride="carousel">
                <!-- Indicators -->
                @if($hasGallery && $Images->count() > 1)
                ...
                <div class="carousel-inner" role="listbox">
                    @if($hasGallery)
                        @foreach ($Images as $key => $Image)
                        <div class="carousel-item {{ $loop->first ? 'active' : '' }}" style="background-image: url({{ asset($Image->image) }});">
                        </div>
                        @endforeach
                    @else
                        <div class="carousel-item active" style="background-image: url({{ $mainImg }});">
                        </div>
                    @endif
                ...
                <a href="#carousel" class="carousel-control-prev" role="button" data-slide="prev">
                    <img src="{{ asset('assets/frontend/img/project-pv.png') }}" alt="">
                    <span class="sr-only"></span>
                </a>
                <a href="#carousel" class="carousel-control-next" role="button" data-slide="next">
                    <img src="{{ asset('assets/frontend/img/project-next.png') }}" alt="">
                    <span class="sr-only"></span>
                </a>
                @endif
                            ...
                            <div class="container">
                                <div class="row align-items-center">
                                    <div class="col-12 col-sm-6">
                                        <h5>Segment : {{ $ProjectsReferenceMain->seament_en ?? '-' }}</h5>
                                        <h1 class="text-left">{{ $ProjectsReferenceMain->tilte_en ?? '-' }}</h1>
                                        <br>
                                        <label class="mt-2 mb-0 float-left">Project Owner : {{ $ProjectsReferenceMain->project_owner_en ?? '-' }}</label>
                                    </div>
                                    <div class="col-6 col-sm-2">
                                        <img src="{{ asset('assets/frontend/img/bath.png') }}" alt="">
                                        <br>
                                        <label class="mt-2">Project Value</label>
                                        <h1>{{ $ProjectsReferenceMain->project_value ?? '-' }}</h1>
                                        <label for="">Million Baht</label>
                                    </div>
                                    <div class="col-6 col-sm-2">
                                        <img src="{{ asset('assets/frontend/img/start.png') }}" alt="">
                                        <br>
                                        <label class="mt-2">Start Project</label>
                                        <h1>{{ $ProjectsReferenceMain->project_start ?? '-' }}</h1>
                                        <label for="">{{ $ProjectsReferenceMain->project_start_month ?? '' }}</label>
                                    </div>
                                    <div class="col-12 col-sm-2">
                                        <img src="{{ asset('assets/frontend/img/Completion.png') }}" alt="">
                                        <br>
                                        <label class="mt-2">Completion</label>
                                        <h1>{{ $ProjectsReferenceMain->project_completion ?? '-' }}</h1>
                                        <label for="">{{ $ProjectsReferenceMain->project_completion_month ?? '' }}</label>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="container-fluid">
                        <div class="caption">
                            <div class="container">
                                <div class="row align-items-center">
                                    <div class="col-12 col-sm-6">
                                        <h5>Segment : {{ $ProjectsReferenceMain->seament_th ?? '-' }}</h5>
                                        <h1 class="text-left">{{ $ProjectsReferenceMain->tilte_th ?? '-' }}</h1>
                                        <br>
                                        <label class="mt-2 mb-0 float-left">Project Owner : {{ $ProjectsReferenceMain->project_owner_th ?? '-' }}</label>
                                    </div>
                                    <div class="col-6 col-sm-2">
                                        <img src="{{ asset('assets/frontend/img/bath.png') }}" alt="">
                                        <br>
