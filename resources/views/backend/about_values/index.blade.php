...
<div class="card cardboby">
    <form action="{{ route('aboutusvalues.index') }}" method="GET" onsubmit="return checkdate()">
        <div class="container-fluid mt-4">
            <div class="row">
                ...
                <div class="text-right col-md-2">
                    <br />
                    <a class="mt-2 ml-0 btn w-100 btn-outline-success" href="{{ route('aboutusvalues.create') }}"><i class="fas fa-plus mr-1"></i>เพิ่มข้อมูล</a>
                </div>
            </div>
        </div>
    </form>
            ...
            <table class="table table-bordered table-striped align-items-center no-wrap">
                <thead>
                    <tr class="text-center">
                        ...
                        <th width="25%">หัวข้อ Values (TH / EN)</th>
                        <th width="35%">รายละเอียด (Detail)</th>
                        <th width="8%">ลำดับ</th>
                        <th width="15%">Action</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($AboutUsValues as $key => $data)
                    <tr class="text-center">
                        <td class="font-weight-bold">{{ $AboutUsValues->firstItem() + $key }}</td>
                        <td>
                            @if($data->image_main && $data->image_main != 'assets/backend/images/error/nopic.jpg')
                                <img src="{{ asset($data->image_main) }}" alt="Values" style="max-height: 55px; max-width: 90px; object-fit: cover; border-radius: 4px;" class="shadow-sm border">
                            @else
                        ...
                        <td class="text-left text-muted">
                            <small>{!! \Illuminate\Support\Str::limit(strip_tags($data->detail_th ?: $data->detail_en), 100, '...') !!}</small>
                        </td>
                        <td>
                            <span class="badge badge-pill badge-info px-3 py-1 font-weight-bold">{{ $data->sort_number }}</span>
                        </td>
                        <td>
                            <a class="btn btn-sm btn-outline-warning mr-1" href="{{ route('aboutusvalues.edit', $data->id) }}">
                                <i class="fas fa-edit mr-1"></i>แก้ไข
                            </a>

                            <form action="{{ route('aboutusvalues.destroy', $data->id) }}" method="POST" class="d-inline" onsubmit="return confirm('คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูล {{ strip_tags($data->tilte_th) }}?');">
                                @csrf
                                @method('DELETE')
    ...
    <div class="card-footer py-3 d-flex justify-content-between align-items-center bg-transparent border-0">
        <div class="text-muted small">
            แสดง {{ $AboutUsValues->firstItem() ?? 0 }} ถึง {{ $AboutUsValues->lastItem() ?? 0 }} จากทั้งหมด {{ $AboutUsValues->total() }} รายการ
        </div>
        <ul class="pagination pagination-primary mb-0">
            {{ $AboutUsValues->onEachSide(0)->withQueryString()->links() }}
        </ul>
    </div>
</div>
@endsection
