...
<div class="card cardboby">
    <form action="{{ route('banner.index') }}" method="GET" onsubmit="return checkdate()">
        <div class="container-fluid mt-4">
            <div class="row">
                ...
                <div class="text-right col-md-2">
                    <br />
                    <a class="mt-2 ml-0 btn w-100 btn-outline-success" href="{{ route('banner.create') }}">เพิ่มข้อมูล</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <div class="card-body">
        <table id="example" class="table table-dark table-bordered table-striped no-wrap responsive">
            ...
            <tbody>
                ...
                @php
                $keysOnclick = $Banner->firstItem() + $key;
                @endphp
                <tr class="text-center" width="100%">
                    <th>{{ $Banner->firstItem() + $key }}</th>
                    <th><img width='20%' src="{{ asset($data->image_banner_slide) }}"></th>
                    <th>{{$data->tilte_th ?? $data->tilte_en ?? '-'}} </th>
                    <th width="20%">
                        {{-- <a class="btn btn-sm btn-outline-info" href="{{url('/banner' . '/' . $data->id)}}">รายละเอียด</a> --}}
                        <a class="btn btn-sm btn-outline-warning" href="{{url('/banner' . '/' . $data->id . '/' . 'edit')}}">แก้ไข</a>
    ...
    <ul class="mr-3 justify-content-end pagination pagination-primary flex_row-center wrap ">
        {{ $Banner->onEachSide(0)->withQueryString()->links() }}
    </ul>
</div>
@endsection
@section('javascript')
    ...
    function updateHiddenDisplay(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '/banner/updateHiddenDisplay',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
