...
<div class="card cardboby">
    <form action="{{ route('aboutus45001.index') }}" method="GET" onsubmit="return checkdate()">
        <div class="container-fluid mt-4">
            <div class="row">
                ...
                <div class="text-right col-md-2">
                    <br />
                    <a class="mt-2 ml-0 btn w-100 btn-outline-success" href="{{ route('aboutus45001.create') }}"><i class="fas fa-plus mr-1"></i>เพิ่มข้อมูล</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered table-striped align-items-center no-wrap">
                ...
                <tbody>
                    @forelse($AboutUs45001 as $key => $data)
                    <tr class="text-center">
                        <td class="font-weight-bold">{{ $AboutUs45001->firstItem() + $key }}</td>
                        <td>
                            @if($data->image1 && $data->image1 != 'Unknown' && $data->image1 != 'assets/backend/images/error/nopic.jpg')
                            <img src="{{ asset($data->image1) }}" alt="ISO 45001 Cert 1" class="cert-thumb shadow-sm">
                            @else
                        ...
                        <td>
                            ...
                            <img src="{{ asset($data->image2) }}" alt="ISO 45001 Cert 2" class="cert-thumb shadow-sm">
                            @else
                        ...
                        <td>
                            <a class="btn btn-sm btn-outline-warning mr-1" href="{{ route('aboutus45001.edit', $data->id) }}">
                                <i class="fas fa-edit mr-1"></i>แก้ไข
                            </a>

                            <form action="{{ route('aboutus45001.destroy', $data->id) }}" method="POST" class="d-inline" onsubmit="return confirm('คุณแน่ใจหรือไม่ว่าต้องการลบใบรับรองรายการนี้?');">
                                @csrf
                                @method('DELETE')
    ...
    <div class="card-footer py-3 d-flex justify-content-between align-items-center bg-transparent border-0">
        <div class="text-muted small">
            แสดง {{ $AboutUs45001->firstItem() ?? 0 }} ถึง {{ $AboutUs45001->lastItem() ?? 0 }} จากทั้งหมด {{ $AboutUs45001->total() }} รายการ
        </div>
        <div>
            {{ $AboutUs45001->withQueryString()->links('pagination::bootstrap-4') }}
        </div>
    </div>
</div>
@endsection
