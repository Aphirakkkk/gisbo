...
<div class="card cardboby">
    <form action="{{ route('footer.index') }}" method="GET" onsubmit="return checkdate()">
        <div class="container-fluid mt-4">
            <div class="row">
                ...
                <div class="text-right col-md-2">
                    <br />
                    <a class="mt-2 ml-0 btn w-100 btn-outline-success" href="{{ route('footer.create') }}">เพิ่มข้อมูล</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <div class="card-body">
        <table id="example" class="table table-dark table-bordered table-striped no-wrap responsive">
            ...
            <tbody>
                ...
                @php
                $keysOnclick = $Footer->firstItem() + $key;
                @endphp
                <tr class="text-center" width="100%">
                    <th>{{ $Footer->firstItem() + $key }}</th>
                    <th>{{$data->tilte_th ?? $data->tilte_en ?? '-'}}</th>
                    <th>
                        @if($data->type_footer == 1)
                        ที่อยู่
                        @elseif($data->type_footer == 2)
                        เบอร์โทรศัพท์
                        @elseif($data->type_footer == 3)
                        โทรสาร
                        @elseif($data->type_footer == 4)
                        E-mail
                        @elseif($data->type_footer == 5)
                        facebook
                        @elseif($data->type_footer == 6)
                        ไลน์
                        @endif
                    </th>
                    <th width="20%">
                        <a class="btn btn-sm btn-outline-warning" href="{{url('/footer' . '/' . $data->id . '/' . 'edit')}}">แก้ไข</a>
    ...
    <ul class="mr-3 justify-content-end pagination pagination-primary flex_row-center wrap ">
        {{ $Footer->onEachSide(0)->withQueryString()->links() }}
    </ul>
</div>
@endsection
@section('javascript')
    ...
    function updateHiddenDisplay(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '/footer/updateHiddenDisplay',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
