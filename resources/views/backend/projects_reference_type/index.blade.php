...
<div class="card cardboby">
    <form action="{{ route('projectsreferencetype.index') }}" method="GET" onsubmit="return checkdate()">
        <div class="container-fluid mt-4">
            <div class="row">
                ...
                {{-- <div class="text-right col-md-2">
                    <br />
                    <a class="mt-2 ml-0 btn w-100 btn-outline-success" href="{{ route('projectsreferencetype.create') }}">เพิ่มข้อมูล</a>
            </div> --}}
        </div>
</div>
</form>
    ...
    <table id="example" class="table table-dark table-bordered table-striped no-wrap responsive">
        <thead>
            <tr class="text-center" width="100%">
                <th width="5%">#</th>
                <th width="75%">Projects Reference Type</th>
                <th width="20%">Action</th>
            </tr>
        </thead>
        <tbody>
            @foreach($ProjectsReferenceType as $key => $data)
            @php
            $keysOnclick = $ProjectsReferenceType->firstItem() + $key;
            @endphp
            <tr class="text-center" width="100%">
                <th>{{ $ProjectsReferenceType->firstItem() + $key }}</th>
                <th>{!! $data->tilte_th ?? $data->tilte_en ?? '-'!!} </th>
                <th width="20%">
                    <a class="btn btn-sm btn-outline-warning" href="{{url('/projectsreferencetype' . '/' . $data->id . '/' . 'edit')}}">แก้ไข</a>
...
<ul class="mr-3 justify-content-end pagination pagination-primary flex_row-center wrap ">
    {{ $ProjectsReferenceType->onEachSide(0)->withQueryString()->links() }}
</ul>
</div>
@endsection
@section('javascript')
    ...
    function updateHiddenDisplay(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '/projectsreferencetype/updateHiddenDisplay',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
    ...
    function updateShowDisplay(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '/projectsreferencetype/updateShowDisplay',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
    ...
    function updateDelete(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                     type:"POST",
                     url:'/projectsreferencetype/updateDelete',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}',
                        },
