...
<div class="card cardboby">
    <form action="{{ route('aboutuspolicy.index') }}" method="GET" onsubmit="return checkdate()">
        <div class="container-fluid mt-4">
            <div class="row">
                ...
                <div class="text-right col-md-2">
                    <br />
                    <a class="mt-2 ml-0 btn w-100 btn-outline-success" href="{{ route('aboutuspolicy.create') }}"><i class="fas fa-plus mr-1"></i>เพิ่มข้อมูล</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered table-striped align-items-center no-wrap">
                ...
                <tbody>
                    @forelse($AboutUsPolicy as $key => $data)
                    <tr class="text-center">
                        <td class="font-weight-bold">{{ $AboutUsPolicy->firstItem() + $key }}</td>
                        <td class="text-left">
                        ...
                        <td>
                            ...
                            <img src="{{ asset($data->image1) }}" alt="Policy 1" class="cert-thumb shadow-sm">
                            @else
                        ...
                        <td>
                            ...
                            <img src="{{ asset($data->image2) }}" alt="Policy 2" class="cert-thumb shadow-sm">
                            @else
                            <span class="text-muted small">- ไม่มีรูป -</span>
                            @endif
                        </td>
                        ...
                        <td>
                            <a class="btn btn-sm btn-outline-warning mr-1" href="{{ route('aboutuspolicy.edit', $data->id) }}">
                                <i class="fas fa-edit mr-1"></i>แก้ไข
                            </a>

                            <form action="{{ route('aboutuspolicy.destroy', $data->id) }}" method="POST" class="d-inline" onsubmit="return confirm('คุณแน่ใจหรือไม่ว่าต้องการลบรายการนี้?');">
                                @csrf
                                @method('DELETE')
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-4 text-muted">
                            <i class="fas fa-file-contract fa-2x mb-2 d-block text-muted"></i>
                            ไม่พบข้อมูล Policy
                        </td>
                    </tr>
                    @endforelse
    ...
    <div class="card-footer py-3 d-flex justify-content-between align-items-center bg-transparent border-0">
        <div class="text-muted small">
            แสดง {{ $AboutUsPolicy->firstItem() ?? 0 }} ถึง {{ $AboutUsPolicy->lastItem() ?? 0 }} จากทั้งหมด {{ $AboutUsPolicy->total() }} รายการ
        </div>
        <div>
            {{ $AboutUsPolicy->withQueryString()->links('pagination::bootstrap-4') }}
        </div>
    </div>
</div>
@endsection
