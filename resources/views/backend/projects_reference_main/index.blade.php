...
<div class="card cardboby">
    <form action="{{ route('projectsreferencemain.index') }}" method="GET" onsubmit="return checkdate()">
        <div class="container-fluid mt-4">
            <div class="row">
                ...
                <div class="text-right col-md-2">
                    <br />
                    <a class="mt-2 ml-0 btn w-100 btn-outline-success" href="{{ route('projectsreferencemain.create') }}">เพิ่มข้อมูล</a>
                </div>
            </div>
        </div>
    </form>
    <!-- /.card-header -->
    <div class="card-body">
        <table id="DataTable" class="table table-bordered table-striped no-wrap responsive">
            <thead>
                <tr class="text-center" width="100%">
                    <th width="5%">#</th>
                    <th width="65%">Projects Reference</th>
                    <th width="30%">Action</th>
                </tr>
            </thead>
            <tbody>
                @php
                $no = isset($_GET['page']) ? 10 * ($_GET['page'] - 1) : 0;
                @endphp
                @foreach($ProjectsReferenceMain as $indexKey => $data)

                <tr class="text-center" width="100%">
                    <th>{{ $indexKey + 1 + $no }}</th>
                    <th>{{$data->tilte_th ?? $data->tilte_en ?? '-'}} (ประเภท Projects Reference {!! $data->portfolioType->tilte_th ?? $data->portfolioType->tilte_en ?? '-'!!})</th>
                    <th width="20%">

                        <a class="btn btn-sm btn-outline-warning" href="{{url('/projectsreferencemain' . '/' . $data->id . '/' . 'edit')}}">แก้ไข</a>

                        <button class='btn btn-sm btn-outline-danger' onclick="updateDelete({{$indexKey}},{{$data->id}})">ลบ</button>
                    </th>
                </tr>
                @endforeach
            </tbody>
    ...
    {{-- <ul class="mr-3 justify-content-end pagination pagination-primary flex_row-center wrap ">
        {{ $ProjectsReferenceMain->onEachSide(0)->withQueryString()->links() }}
    </ul> --}}
</div>
@endsection
@section('javascript')
    ...
    function updateHiddenDisplay(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '/projectsreferencemain/updateHiddenDisplay',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
    ...
    function updateShowDisplay(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '/projectsreferencemain/updateShowDisplay',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
    ...
    function updateDelete(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                     type:"POST",
                     url:'/projectsreferencemain/updateDelete',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}',
                        },
