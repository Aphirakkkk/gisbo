...
<div class="card cardboby">
    <form action="{{ route('aboutus.index') }}" method="GET" onsubmit="return checkdate()">
        <div class="container-fluid mt-4">
            <div class="row">
                ...
                <div class="text-right col-md-2">
                    <br />
                    <a class="mt-2 ml-0 btn w-100 btn-outline-success" href="{{ route('aboutus.create') }}"><i class="fas fa-plus mr-1"></i>เพิ่มข้อมูล</a>
                </div>
            </div>
        </div>
    </form>
            ...
            <table class="table table-bordered table-striped align-items-center no-wrap">
                <thead>
                    <tr class="text-center">
                        <th width="5%">#</th>
                        <th width="30%">หัวข้อ About Us Home (TH / EN)</th>
                ...
                <tbody>
                    @forelse($AboutUsMain as $key => $data)
                    <tr class="text-center">
                        <td class="font-weight-bold">{{ $AboutUsMain->firstItem() + $key }}</td>
                        <td class="text-left">
                        ...
                        <td>
                            <a class="btn btn-sm btn-outline-warning mr-1" href="{{ route('aboutus.edit', $data->id) }}">
                                <i class="fas fa-edit mr-1"></i>แก้ไข
                            </a>

                            <form action="{{ route('aboutus.destroy', $data->id) }}" method="POST" class="d-inline" onsubmit="return confirm('คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูล {{ strip_tags($data->tilte_th) }}?');">
                                @csrf
                                @method('DELETE')
    ...
    <div class="card-footer py-3 d-flex justify-content-between align-items-center bg-transparent border-0">
        <div class="text-muted small">
            แสดง {{ $AboutUsMain->firstItem() ?? 0 }} ถึง {{ $AboutUsMain->lastItem() ?? 0 }} จากทั้งหมด {{ $AboutUsMain->total() }} รายการ
        </div>
        <ul class="pagination pagination-primary mb-0">
            {{ $AboutUsMain->onEachSide(0)->withQueryString()->links() }}
        </ul>
    </div>
</div>
@endsection
