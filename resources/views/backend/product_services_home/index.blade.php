...
<div class="card cardboby">
    <form action="{{ route('productserviceshome.index') }}" method="GET" onsubmit="return checkdate()">
        <div class="container-fluid mt-4">
            <div class="row">
                ...
                {{-- <div class="text-right col-md-2">
                    <br />
                    <a class="mt-2 ml-0 btn w-100 btn-outline-success" href="{{ route('productserviceshome.create') }}">เพิ่มข้อมูล</a>
            </div> --}}
        </div>
</div>
</form>
...
<div class="card-body">
    <table id="example" class="table table-dark table-bordered table-striped no-wrap responsive">
        ...
        <tbody>
            @foreach($ProductServicesHome as $key => $data)
            @php
            $keysOnclick = $ProductServicesHome->firstItem() + $key;
            @endphp
            <tr class="text-center" width="100%">
                <th>{{ $ProductServicesHome->firstItem() + $key }}</th>
                <th>{{$data->tilte_th ?? $data->tilte_en ?? '-'}} </th>
                <th width="20%">
                    <a class="btn btn-sm btn-outline-warning" href="{{url('/productserviceshome' . '/' . $data->id . '/' . 'edit')}}">แก้ไข</a>
...
<ul class="mr-3 justify-content-end pagination pagination-primary flex_row-center wrap ">
    {{ $ProductServicesHome->onEachSide(0)->withQueryString()->links() }}
</ul>
</div>
@endsection
@section('javascript')
    ...
    function updateHiddenDisplay(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '/productserviceshome/updateHiddenDisplay',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
    ...
    function updateShowDisplay(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '/productserviceshome/updateShowDisplay',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
    ...
    function updateDelete(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                     type:"POST",
                     url:'/productserviceshome/updateDelete',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}',
                        },
