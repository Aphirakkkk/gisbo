...
<div class="card cardboby">
    <form action="{{ route('careermain.index') }}" method="GET" onsubmit="return checkdate()">
        <div class="container-fluid mt-4">
            <div class="row">
                ...
                <div class="text-right col-md-2">
                    <br />
                    <a class="mt-2 ml-0 btn w-100 btn-outline-success" href="{{ route('careermain.create') }}">เพิ่มข้อมูล</a>
                </div>
            </div>
        </div>
    </form>
        ...
        <table id="example" class="table table-dark table-bordered table-striped no-wrap responsive">
            <thead>
                <tr class="text-center" width="100%">
                    <th width="5%">#</th>
                    <th width="65%">หัวข้อ Career ส่วนหลัก</th>
                    <th width="30%">Action</th>
                </tr>
            </thead>
            <tbody>
                @foreach($CareerMain as $key => $data)
                @php
                $keysOnclick = $CareerMain->firstItem() + $key;
                @endphp
                <tr class="text-center" width="100%">
                    <th>{{ $CareerMain->firstItem() + $key }}</th>
                    <th>{{$data->tilte_th ?? $data->tilte_en ?? '-'}}</th>
                    <th width="20%">

                        <a class="btn btn-sm btn-outline-warning" href="{{url('/careermain' . '/' . $data->id . '/' . 'edit')}}">แก้ไข</a>
    ...
    <ul class="mr-3 justify-content-end pagination pagination-primary flex_row-center wrap ">
        {{ $CareerMain->onEachSide(0)->withQueryString()->links() }}
    </ul>
</div>
@endsection
@section('javascript')
    ...
    function updateHiddenDisplay(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '/careermain/updateHiddenDisplay',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
    ...
    function updateShowDisplay(row, id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: '/careermain/updateShowDisplay',
                        data: {
                            id: id,
                            _token: '{{ csrf_token() }}'
                        },
