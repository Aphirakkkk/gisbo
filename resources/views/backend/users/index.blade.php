...
<div class="card cardboby">
    <form action="{{ route('user.index') }}" method="GET" onsubmit="return checkdate()">
        <div class="container-fluid mt-4">
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        <i class="fas fa-search"></i> <label for="search"><b>ค้นหาชื่อ</b></label>
                ...
                <div class="text-right col-md-2">
                    <br />
                    <a class="mt-2 ml-0 btn w-100 btn-outline-success" href="{{ route('user.create') }}">เพิ่มข้อมูล</a>
                </div>
            </div>
        </div>
    </form>
    ...
    <div class="card-body">
        <table id="example" class="table table-dark table-bordered table-striped no-wrap responsive">
            ...
            <tbody>
                ...
                @php
                $keysOnclick = $User->firstItem() + $key;
                @endphp
                <tr class="text-center" width="100%">
                    <th>{{ $User->firstItem() + $key }}</th>
                    <th>{{$data->fullname}} <a href="{{url('/user' . '/' . $data->id)}}">(รายละเอียด)</a></th>
                    <th>{{$data->created_at}} </th>
                    <th>{{$data->updated_at}} </th>
                    <th width="20%">
                        <a class="btn btn-sm btn-outline-warning" href="{{url('/user' . '/' . $data->id . '/' . 'edit')}}">แก้ไข</a>
    ...
    <ul class="mr-3 justify-content-end pagination pagination-primary flex_row-center wrap ">
        {{ $User->onEachSide(0)->withQueryString()->links() }}
    </ul>
</div>
@endsection
@section('javascript')
